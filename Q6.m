...
% Date: 18 May 2018
% -----
function [mse,ntres] = Q6(param, Np, y_real, sigmaObs, n_toplot)

mse = zeros(1,n_toplot);
N_tres = linspace(0,Np,n_toplot);
ntres = N_tres./Np;
y= make_observations(param, y_real, sigmaObs);
...
for test = 1:n_toplot;
    ...
    for k = 1:param.itmax+1 %for each time step
        ...
        if 1/sum(weights.^2) < N_tres(test)
